...
class as637(AbstractBot):
    ...
    def get_move(self, board: ConnectFourBoard) -> int:
        ...
        if move_count < 14:
            ...
            for move in valid_moves:
                ...
                if (
                    future_board.is_victory
                    and future_board.winner == board.current_player
                ):
                    return move
    ...
    def find_winning_move(
        self, board: ConnectFourBoard, valid_moves: list[int]
    ) -> int | None:
    ...
    def find_fork_move(
        ...
        for move in valid_moves:
            ...
            for next_move in future_board.get_valid_moves():
                ...
                if (
                    next_future.is_victory
                    and next_future.winner == board.current_player
                ):
    ...
    def minimax_search(self, board: ConnectFourBoard, valid_moves: list[int]) -> int:
        ...
        for current_depth in range(1, self.MAX_DEPTH + 1):
            ...
            for move in ordered_moves:
                ...
                future_board = board.make_move(move)
                score = -self.minimax(
                    future_board, current_depth - 1, -beta, -alpha, start_time, 1
                )
    ...
    def minimax(
        ...
        for move in ordered_moves:
            ...
            future_board = board.make_move(move)
            score = -self.minimax(
                future_board, depth - 1, -beta, -alpha, start_time, ply + 1
            )
    ...
    def evaluate_connectivity(
        self, board: ConnectFourBoard, player: CellState
    ) -> float:
        ...
        for row in range(board.num_rows - 1):
            for col in range(board.num_cols - 1):
                ...
                if (
                    row > 0
                    and board[col, row] == player
                    and board[col + 1, row - 1] == player
                ):
                    score += 1
